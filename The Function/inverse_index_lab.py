...
def movie_review(name):
    ...
    return review[randint(0, len(review))]
...
def makeInverseIndex(strlist):
    ...
    for (k, v) in enumerate(strlist):
        for word in v.split():
            if word not in dict_index:
                dict_index[word] = set()
            dict_index[word].add(k)
    return dict_index

#Solution: Found out this runs faster than the other soln above
#    inverseIndex = {}
...
def orSearch(inverseIndex, query):
    ...
    set_value = set()
    for value in query:
        #if value in inverseIndex.keys(): Just for precautionary measures
            for id in inverseIndex[value]:
                set_value.add(id)
    return set_value
...
def andSearch(inverseIndex, query):
    ...
    """
    word_and_set = set()
    word_and_set = inverseIndex[query[0]]
    for word in query:
        word_and_set = word_and_set & inverseIndex[word]
    return word_and_set