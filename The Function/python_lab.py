...
divisible_by_3 = (673 + 909) % 3 == 0 #True and ((673 + 909) / 3 == 0)
...
#Explanation, the expression execute the else statement when the condition
#..was not met assuming x = -11 then the 1st expression before if statment is
#....executed
...
#value = {x * y for x in {3,5,7} for y in {2, 4, 6}}
value = {x * y for x in X1 for y in Y1}
...
#value2 = {x * y for x in {3,4,6} for y in {2, 5, 7} if x < y}
value2 = {x * y for x in X2 for y in Y2 if x < y}
...
#In some cases for could be in the last call
#Couldnt get the list comprehension, later got the solution as expressed above
#for x in dlist:
#    if k in x:
#        print(x[k])
#    else:
...
square_dict = value = {x: x**2 for x in range(100)}
...
def all_3_digit_numbers(base, digits): 
    return {x * base**2 + y * base + z for x in digits for y in \
            digits for z in digits}
#THIS IS A VERY IMPORTANT PART TO NOTE COS THERE IS AN ITERATION OVER POSSIBLE
#...COMBINATION OF THE ELEMENTS IN DIGITS
